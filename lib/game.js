...
newGameButton.addEventListener('click', setZeroState);
  ...
  if (isGameOver) {
    ...
    if (!paused) {
      if (currentTime - startingTime > 1000 / fps) {
        ...
        player1.draw(context, p1Color, player2.history, canvas);
        player2.draw(context, p2Color, player1.history, canvas);
      }
    }
